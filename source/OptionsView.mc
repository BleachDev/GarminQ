...
class OptionsView extends View {
    ...
    function onUpdate(dc as Dc) as Void {
        ...
        dc.drawText(W / 2.7, H / 4 + hs + XTINY_H * 2, Graphics.FONT_MEDIUM, "Gap:", Graphics.TEXT_JUSTIFY_RIGHT);

        drawValue(dc, W / 2.4, H / 4, data.statsMode == 0 ? "Records" : data.statsMode == 1 ? "7 Day Avg" : "1 Day Avg", selected == 0, Graphics.FONT_MEDIUM);
        drawValue(dc, W / 2.4, H / 4 + hs, data.inActivity ? "Yes" : "No", selected == 1, Graphics.FONT_MEDIUM);
        drawValue(dc, W / 2.4, H / 4 + hs + XTINY_H * 2, data.distMode == 0 ? "Normal" : "Exponential", selected == 2, Graphics.FONT_MEDIUM);
